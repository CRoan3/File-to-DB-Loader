...
def read_csv(file, schemas):
    ...
    )  # instead of creating dataframe by reading CSV file assigning column names, we are getting df reader
    return df_reader
...
def process_files(
    ds_names=None,
):  # not passing an argument will attempt to process all files
    ...
    if (
        not ds_names
    ):  # if no ds_names given, it processes all datasets based on keys available as part of schemas
        ds_names = schemas.keys()
    for ds_name in ds_names:
        try:
            print(f'Processing {ds_name}')
            db_loader(
                src_base_dir, db_conn_url, ds_name
            )  # reading data to df in chunks, then they are returned to respective table in db
